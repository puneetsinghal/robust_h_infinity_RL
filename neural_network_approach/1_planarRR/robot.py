...
class Acrobot(object):
	...
	def generateSystemData(self, control_generator):
		...
		# file = h5py.File('Acrobot_systemData3.h5', 'w') 
		# file.create_dataset('U', data=U)
		# file.create_dataset('W', data=W)
		# file.create_dataset('X', data=X)
		# file.close()

		U = spio.loadmat('U.mat', squeeze_me=True)
		X = spio.loadmat('X.mat', squeeze_me=True)	
		W = spio.loadmat('W.mat', squeeze_me=True)
		U = U['U']
		X = X['X']
		W = W['W']	
	...
	def dynamics_test(self, t, x):
		...
		grad = self.sess.run(self.nn.value_grad_t,feed_dict={self.nn.X_t:x, self.nn.X_tPlus:x})
...
class PlanarRR(object):
	...
	def dynamics_test(self, t, x):
		...
		grad = self.sess.run(self.nn.value_grad_t,feed_dict={self.nn.X_t:x, self.nn.X_tPlus:x, self.nn.dropout_prob:1.0})
		# embed()
		ut   = -0.5*np.matmul(g, grad.reshape(self.DIM,1)).reshape(self.numInputs,)
		# print(ut)
		# d   = 4*np.exp(-0.1*t)*sin(t)
		wt   = 0.01*np.exp(-0.1*t)*sin(t)*np.ones(2)
	#     print(u)
		# embed()
		xdot = f + np.matmul(g.T,ut) + np.matmul(k.T, wt)
	#     print(xdot)
