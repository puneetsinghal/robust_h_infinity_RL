...
%     u      = 0.5*control_generator*rand(size(control_generator,2),1);
    u      = control_generator(:,j);
    
    w      = 0.5*rand(length(tspan),1);
%     x0     = rand(4,1);
    [~,x]  = ode45(@dynamicsknown,tspan,x0);
...
%     if(rem(t,1000) == 0 && floor(t*100/length(X)) >= 100)
%         save matrices_data.mat rho_delphi rho_gdelphi rho_kdelphi rho_uphi rho_wphi rho_h
%         optimize(rho_delphi(1:t,:), rho_gdelphi(1:t,:,:),...
%             rho_kdelphi(1:t,:,:), rho_uphi(1:t,:), rho_wphi(1:t,:), rho_h(1:t,:), phit, t);
